...
class ProductListItem extends React.PureComponent {
    ...
    render() {
        let { id, image, title, navigation, price, rating, wish = true, type, onWishTapped } = this.props;
        let iconName = wish ? "md-heart" : "md-heart-outline";
        if (type === "admin") {
            iconName = wish ? "ios-trash" : "ios-trash-outline";
        }
        return (
            <TouchableOpacity
                activeOpacity={0.5}
